...
function render() {
  ...
  board.ontouchstart = e => {
    ...
    startIndex = el && el.classList.contains("tile")
      ? [...board.children].indexOf(el)
      : null;
  };
  ...
  board.ontouchend = e => {
    ...
    handleSlide(startIndex, dir);
    startIndex = null;
  };
}

/* ===== 正しいスライド処理 ===== */
function handleSlide(fromIndex, dir) {
  const empty = tiles.indexOf(0);
  const fr = Math.floor(fromIndex / size);
  const fc = fromIndex % size;
  ...
  if (fr === er) {
    if (dir === "left" && fc > ec) distance = fc - ec;
    if (dir === "right" && fc < ec) distance = ec - fc;
  }

  if (fc === ec) {
    if (dir === "up" && fr > er) distance = fr - er;
    if (dir === "down" && fr < er) distance = er - fr;
  }
  ...
  for (let i = 0; i < distance; i++) {
    ...
    const step =
      ...
      dir === "up"    ? e + size :
                        e - size;
    [tiles[e], tiles[step]] = [tiles[step], tiles[e]];
  }
